...
router.put('/exam/:id', async (req, res) => {
    ...
    let answeredQuestion = exam.answeredQuestions.find(aq => aq._id == req.body.questionId);
    if (answeredQuestion) {
        answeredQuestion.answers = [...req.body.answers];
    }
    else {
        exam.answeredQuestions = [...exam.answeredQuestions, { _id: req.body.questionId, answers: req.body.answers }]
    }
    student.save();
    res.send(student)
    // res.send(query);
